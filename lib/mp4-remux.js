...
var H = (n, t, e) => t in n ? W(n, t, { enumerable: !0, configurable: !0, writable: !0, value: e }) : n[t] = e;
var w = (n, t, e) => (H(n, typeof t != "symbol" ? t + "" : t, e), e);
  ...
  parse(t) {
    ...
    for (; e <= this.stream.length - 8; ) {
      ...
      if (r <= this.stream.length) {
        const a = this.stream.slice(e, e + r), u = M[s] || S;
...
class q extends c {
  ...
  get refList() {
    ...
    for (let s = 0; s < e; s++) {
      ...
      const _ = u >> 31 & 1, p = u & 2147483647, d = this.readUint(4, a + 4);
...
function Y(n, t) {
  const e = n.getReader(), r = t.getReader(), s = new D(), a = new D(), { readable: u, writable: _ } = new TransformStream(), p = _.getWriter();
  let d = -1;
  function b(x) {
    p.write(x);
  }
  async function v() {
    if (d === -1) {
      ...
      if (x && g) {
        const i = x.mvex, f = g.mvex;
        if (i && f) {
          const o = i.trex, l = f.trex;
    ...
    if (!(!s.sidx || !a.sidx) && d !== -1) {
      const x = s.sidx.reference_count, g = a.sidx.reference_count;
      if (d <= x && d <= g) {
        const i = s.boxs.find((o) => o instanceof k && (o == null ? void 0 : o.mfhd.sequence_number) === d), f = a.boxs.find((o) => o instanceof k && (o == null ? void 0 : o.mfhd.sequence_number) === d), h = i && s.boxs[s.boxs.indexOf(i) + 1], U = f && a.boxs[a.boxs.indexOf(f) + 1];
        if (i && h && f && U) {
          const o = i.traf, l = f.traf;
          if (o.tfhd.track_id = 1, l.tfhd.track_id = 2, o != null && o.trun && (l != null && l.trun)) {
            const z = l.size, I = i.size, L = h.size;
            o.trun.data_offset = I + z + 8, l.trun.data_offset = I + z + L, i.boxs.push(l), b(i.raw), h.push(U.raw.slice(8)), b(h.raw), s.boxs = s.boxs.filter((m) => m !== i), s.boxs = s.boxs.filter((m) => m !== h), a.boxs = a.boxs.filter((m) => m !== f), a.boxs = a.boxs.filter((m) => m !== U), d += 1, v();
          }
        }
      } else if (d > x && d <= g) {
        const i = a.boxs.find((h) => h instanceof k && (h == null ? void 0 : h.mfhd.sequence_number) === d);
        i.traf.tfhd.track_id = 2;
        const f = i && a.boxs[a.boxs.indexOf(i) + 1];
        i && f && (b(i.raw), b(f.raw), d += 1, v());
      } else if (d > g && d <= x) {
        const i = s.boxs.find((h) => h instanceof k && (h == null ? void 0 : h.mfhd.sequence_number) === d);
        i.traf.tfhd.track_id = 1;
        const f = i && s.boxs[s.boxs.indexOf(i) + 1];
        i && f && (b(i.raw), b(f.raw), d += 1, v());
      } else
        d > x && d > g && (d = -2, p.close());
    }
  ...
  async function y(x, g) {
    for (; ; ) {
      const { done: i, value: f } = await x.read();
      if (f && (g.push(f), g.parse(), v()), i) {
        g.end(), v();
        break;
      }
    }
